...
export const reducer = createReducer(
  ...
  on(
    ...
    (state, { promotionId, product }): PromotionState => ({
      ...
      selectedPromotion: state.selectedPromotion && state.selectedPromotion.id === promotionId
        ? {
            ...state.selectedPromotion,
            products: [...(state.selectedPromotion.products || []), product],
          }
        : state.selectedPromotion,
    }),
  ),
  on(
    ...
    (state, { promotionId, productId }): PromotionState => ({
      ...
      selectedPromotion: state.selectedPromotion && state.selectedPromotion.id === promotionId
        ? {
            ...state.selectedPromotion,
            products: (state.selectedPromotion.products || []).filter(prod => prod.id !== productId),
          }
        : state.selectedPromotion,
    }),
  ),
  on(
    ...
    (state, { promotionId, promotion }): PromotionState => ({
      ...
      selectedPromotion: state.selectedPromotion?.id === promotionId ? promotion : state.selectedPromotion,
    }),
  ),
);
